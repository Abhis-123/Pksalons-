...
if(!empty($state)){
        $query= "SELECT saloon_id,saloon_owner,saloon_title,userId,location,saloon_number,saloon_address,time_start,time_off 
        From saloons 
        Where Home_avail=1
        AND saloon_address LIKE '%{$state}%'";
        ...
        while ($row=mysqli_fetch_assoc($rows)) {
          echo '<div class="saloon_card" id='.$row['saloon_id'].'>
            <div> 
               ...
               <p Class="Saloon_timing-search">Avilability:'.seconds_to_time($row['time_start']).'-'.seconds_to_time($row['time_off']).' </p>
               <button type="submit" class="btnAction_search"><a href="book.php?s_id='.$row['saloon_id'].'&u_id='.$_SESSION['userID'].'&type=1"
